    ...
    for i in range(n):
        ...
        while small_list[11] > r or small_list[11].denominator > r:
            ...
            for j in range(1, i):
                ...
                if small_list[11] == big_list[j][11]:
                    list_1 = paixu(small_list[3:6])
                    list_2 = paixu(big_list[j][3:6])
                    if list_1 == list_2:
                        small_list[11] = r + 1
                        break
...
def paixu(px_list: list):
    for i in range(1, len(px_list)):
        for j in range(1, len(px_list) - i):
            if px_list[j] > px_list[j + 1]:
                px_list[j], px_list[j + 1] = px_list[j + 1], px_list[j]
    return px_list
